...
// RPCAdaptorServer is an instance of RPC service.
type RPCAdaptorServer struct {
	*RPCAdaptorService
...
// NewRPCAdaptorServer creates a new instance of RPCAdaptorServer.
func NewRPCAdaptorServer() *RPCAdaptorServer {
	t := &RPCAdaptorServer{
		RPCAdaptorService: &RPCAdaptorService{
			wg: &sync.WaitGroup{},
		},
	}

	s := rpc.NewServer()
	s.RegisterName("eth", t.RPCAdaptorService)
...
func (t *RPCAdaptorServer) Start(ctx context.Context) {
...
func (t *RPCAdaptorServer) mainLoop() {
	defer t.wg.Done()
...
func (t *RPCAdaptorServer) Stop() {
	t.cancel()
}

// Wait blocks until all goroutines stop.
func (t *RPCAdaptorServer) Wait() {
	t.wg.Wait()
}
